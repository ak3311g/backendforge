...
async function resolveTemplate(
  ...
  const relativeSubpath = path.join(
    "templates",
    language,
    framework,
  ...
  spinner.start(
    `Local template not found. Downloading ${color.cyan(`${language}/${framework}`)} from GitHub...`,
  );

  try {
    // Downloads only the specific subfolder (e.g. templates/typescript/fastify) from GitHub
    await downloadTemplate(
      `${GITHUB_REPO}/templates/${language}/${framework}/${DEFAULT_ARCHITECTURE}`,
      {
        dir: targetDir,
        force: true,
      },
    ...
    spinner.stop(
      color.green(`Downloaded ${language}/${framework} template from GitHub`),
    );
    return "remote";
  } catch (err: any) {
    spinner.stop(color.red("Failed to fetch remote template."));
    throw new Error(
      `Template not found locally and remote download failed: ${err.message}`,
    );
  }
}

// Recursive helper to update module import names across all .go files
async function replaceGoModuleNames(
  dir: string,
  oldModule: string,
  newModule: string,
): Promise<void> {
...
export async function generateProject(
  ...
  if (
    ...
    !(await fs.pathExists(envTargetPath))
  ) {
